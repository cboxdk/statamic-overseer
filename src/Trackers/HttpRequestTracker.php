...
class HttpRequestTracker extends Tracker
{
    public function register($app)
    {
        $app['events']->listen(ConnectionFailed::class, [$this, 'recordFailed']);
        $app['events']->listen(ResponseReceived::class, [$this, 'recordSuccess']);
    }

    public function recordSuccess(ResponseReceived $event)
    {
        if (! Overseer::isTracking()) {
            return;
        }

        $log = new Event('http_request', [
    ...
    public function recordFailed(ConnectionFailed $event)
    {
        if (! Overseer::isTracking()) {
            return;
        }
